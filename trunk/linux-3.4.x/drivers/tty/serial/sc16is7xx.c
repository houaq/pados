...
/* XXX: hard coded clock freq and interrupt GPIO pin */
#define SC16_CLOCK_FREQ	1843200
#define SC16_INTR_PIN	27
...
static void sc16is7xx_handle_tx(struct uart_port *port)
{
	...
	if (likely(to_send)) {
		...
		txlen = sc16is7xx_port_read(port, SC16IS7XX_TXLVL_REG) - 1;
...
static irqreturn_t sc16is7xx_isr(int irq, void *dev_id)
{
	if (ralink_gpio_int_status(SC16_INTR_PIN))
		return IRQ_WAKE_THREAD;
	return ralink_gpio_irq_handler(irq, NULL);
}
				   ...
				   struct serial_rs485 *rs485)
{
	...
	if (one->rs485.flags & SER_RS485_RTS_AFTER_SEND) {
		sc16is7xx_port_update(port, SC16IS7XX_EFCR_REG,
...
static int sc16is7xx_startup(struct uart_port *port)
{
	...
	/* Enable auto RS485 xmit control */
			   ...
			   struct regmap *regmap, int irq, unsigned long flags)
{
	...
	freq = SC16_CLOCK_FREQ;	/* default clock freq */
	...
	/* Setup interrupt */
	ralink_gpio_int_enabled(SC16_INTR_PIN);
	ret = devm_request_threaded_irq(dev, irq, sc16is7xx_isr, sc16is7xx_ist, IRQF_ONESHOT | flags, SC16IS7XX_NAME, s);
	if (ret) {
