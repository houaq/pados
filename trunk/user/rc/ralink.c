...
int
setSerialNo(const char *sn)
{
	char SN[17];

	flash_mtd_write(MTD_PART_NAME_FACTORY, OFFSET_SERIAL_NO, (unsigned char*)sn, 16);
...
getSerialNo(void)
{
	...
	flash_mtd_read(MTD_PART_NAME_FACTORY, OFFSET_SERIAL_NO, SN, 16);
	if (SN[0]!= 0xff)
		puts(SN);
	return 0;
}
