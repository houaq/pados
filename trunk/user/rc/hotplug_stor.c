...
mdev_sd_main(int argc, char **argv)
{
	...
	snprintf(aidisk_cmd, sizeof(aidisk_cmd), "/sbin/automount.sh %s AiDisk_%c%s", device_name, device_name[2], partno);
...
mdev_mmc_main(int argc, char **argv)
{
	...
	snprintf(aidisk_cmd, sizeof(aidisk_cmd), "/sbin/automount.sh %s AiCard_%c%c", device_name, device_name[6], partno);
